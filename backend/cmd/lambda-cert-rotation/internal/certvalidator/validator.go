...
func Validate(bundle Bundle, expectedDomain string, now time.Time) (Result, error) {
	...
	if err := server.VerifyHostname(expectedDomain); err != nil {
		return Result{}, ValidationError{
	...
	// Chain-building check: ensure server verifies up to the provided chain.
	//
	// We intentionally treat the provided intermediates as trust anchors here (Roots),
	// since the actual public roots are environment-specific and are not required to
	// be present in chain.pem for this workflow.
...
func keyMatchesCert(priv crypto.Signer, pub any) error {
	...
	case *ecdsa.PublicKey:
		want, err := x509.MarshalPKIXPublicKey(p)
		if err != nil {
			return err
		}
		...
		got, err := x509.MarshalPKIXPublicKey(gotPub)
		if err != nil {
			return err
		}
